...
def calculate_bigrams(word_list):
    ...
    dict = {}
    bigrams = {}
    dict[None] = word_list[0]
    for i in range(len(word_list) - 1):
        if word_list[i] not in dict: dict[word_list[i]] = []
        dict[word_list[i]].append(word_list[i+1])

    probability_finder(dict, bigrams)
    print(bigrams)
    return bigrams
...
def calculate_trigrams(word_list):
    ...
    dict = {}
    trigrams = {}
    dict[(None, None)] = word_list[0]
    dict[(None, word_list[1])] = word_list[2]
    for i in range(len(word_list) - 2):
        if (word_list[i], word_list[i + 1]) not in dict: 
            dict[(word_list[i], word_list[i + 1])] = []
        dict[(word_list[i], word_list[i + 1])].append(word_list[i+2])

    probability_finder(dict, trigrams)
    print(trigrams)
    return trigrams

def probability_finder(dict, grams):
    for word in dict:
        probs = {i:dict[word].count(i)/len(dict[word]) for i in dict[word]}
        grams[word] = probs
...
if __name__ == "__main__":
    ...
    calculate_trigrams(['i', 'think', 'therefore', 'i', 'am', 'i', 'think', 'i', 'think'])
